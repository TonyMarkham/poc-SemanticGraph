...
use crate::document_symbols::mapper::{
    build_symbol_key, lsp_symbol_kind_name, normalize_symbol_kind, text_range_from_lsp,
};
...
impl CSharpDocumentSymbolMapper {
    pub fn map_response(
    ...
    pub fn map_nested_symbols(
        ...
        let file_contents = fs::read_to_string(&request.file_path).map_err(|source| {
            ExtractError::io(
                "read C# source file for document symbol mapping",
        ...
        for symbol in symbols {
            if is_synthetic_file_root(symbol) {
                if let Some(children) = &symbol.children {
                    for child in children {
                        Self::map_symbol(child, None, &[], &mut extraction)?;
                    }
                }
            } else {
    ...
    fn map_symbol(
        ...
        let raw_json = serde_json::to_value(symbol)
            .map_err(|source| ExtractError::json("serialize csharp-ls document symbol", source))?;
        ...
        extraction.symbols.push(ExtractedSymbol {
            ...
            kind,
            qualified_name: Some(csharp_qualified_name(parent_path, &symbol.name)),
...
fn is_synthetic_file_root(symbol: &DocumentSymbol) -> bool {
    symbol.kind == SymbolKind::FILE
}

fn csharp_qualified_name(parent_path: &[String], name: &str) -> String {
