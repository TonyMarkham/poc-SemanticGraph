...
impl SoulDocumentSymbolMapper {
    ...
    pub fn map_nested_symbols(
        ...
        let file_contents = fs::read_to_string(&request.file_path).map_err(|source| {
            ExtractError::io(
                "read Soul source file for document symbol mapping",
    ...
    fn map_symbol(
        ...
        let raw_json = serde_json::to_value(symbol)
            .map_err(|source| ExtractError::json("serialize soul-lsp document symbol", source))?;
