...
#[test]
fn extracts_rust_calls_from_wip() -> Result<(), Box<dyn Error>> {
...
#[test]
fn persists_call_edges_occurrences_and_evidence() -> Result<(), Box<dyn Error>> {
    run_with_rust_analyzer(async {
        let repo_root = repo_root()?;
        let provider = RustAnalyzerProvider::new();
        let request = call_request(&provider, &repo_root)?;
...
#[test]
fn later_successful_call_run_closes_unobserved_call_edges() -> Result<(), Box<dyn Error>> {
    run_with_rust_analyzer(async {
        let repo_root = repo_root()?;
        let provider = RustAnalyzerProvider::new();
        let request = call_request(&provider, &repo_root)?;
...
fn call_request(
    ...
    repo_root: &Path,
) -> Result<CallBatchRequest, Box<dyn Error>> {
