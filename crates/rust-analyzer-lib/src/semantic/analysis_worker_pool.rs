...
pub struct AnalysisWorkerPool {
    workers: Arc<Vec<AnalysisWorkerHandle>>,
...
impl AnalysisWorkerPool {
    pub fn start(
        ...
        if worker_count == 0 {
            return Err(RustAnalyzerLibError::analysis_message(
                "start analysis worker pool",
                "analysis worker pool must contain at least one worker",
            ));
        }
                ...
                Err(_panic) => {
                    if first_error.is_none() {
                        first_error = Some(RustAnalyzerLibError::analysis_message(
                            "start analysis worker pool",
                            "analysis worker startup thread panicked",
                        ));
                    }
                }
            }
    ...
    pub fn worker_handles(&self) -> Vec<AnalysisWorkerHandle> {
    ...
    fn next_worker(&self) -> RustAnalyzerLibResult<AnalysisWorkerHandle> {
        ...
        self.workers.get(index).cloned().ok_or_else(|| {
            RustAnalyzerLibError::analysis_message(
                "select analysis worker",
                "analysis worker pool contained no workers",
            )
        })
    }
}
...
fn document_symbol_join_error(error: JoinError) -> RustAnalyzerLibError {
    RustAnalyzerLibError::analysis_message(
        "join analysis worker pool document symbol task",
        error.to_string(),
    )
}
...
fn shutdown_join_error(error: JoinError) -> RustAnalyzerLibError {
    RustAnalyzerLibError::analysis_message(
        "join analysis worker pool shutdown task",
        error.to_string(),
    )
}
